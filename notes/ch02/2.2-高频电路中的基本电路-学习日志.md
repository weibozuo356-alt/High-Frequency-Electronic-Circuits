...
| 回路 | 方便的量 | 相加规则 | 谐振条件 |
| --- | --- | --- | --- |
| 串联 | 阻抗 $`Z=R+jX`$ | $`Z_\Sigma=Z_1+Z_2+\cdots`$ | $`\operatorname{Im}(Z)=X=0`$ |
| 并联 | 导纳 $`Y=G+jB=1/Z`$ | $`Y_\Sigma=Y_1+Y_2+\cdots`$ | $`\operatorname{Im}(Y)=B=0`$ |
...
- [ ] 能从 $`\operatorname{Im}(Z)=0`$ 推出串联谐振条件。
- [ ] 能从 $`\operatorname{Im}(Y)=0`$ 推出并联谐振条件。
